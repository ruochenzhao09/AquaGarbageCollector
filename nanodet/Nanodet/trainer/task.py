...
class TrainingTask(LightningModule):
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        if all_results:
            ...
            # save normal model by wyy
            normal_save_path = os.path.join(self.cfg.save_dir, str(self.current_epoch+1) + "_model")
            mkdir(self.local_rank, normal_save_path)
            self.trainer.save_checkpoint(
                os.path.join(normal_save_path, str(self.current_epoch+1) + "_model.ckpt")
            )
            self.save_model_state(
                os.path.join(normal_save_path, str(self.current_epoch+1) + "_nanodet_model.pth")
            )
